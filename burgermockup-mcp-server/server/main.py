...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "3. **generate_mockups(job_id, design_id, product_id, scene_specs, n, placement)** — "
        "generate mockups. Requires both `design_id` (step 2) and `product_id` (step 1). "
        "Use a fresh UUID for `job_id`. `n` defaults to 1; pass n>1 ONLY when the user asks "
        "for a number of images (max 8). `placement` positions the design: center (default), "
        "chest, left-chest, right-chest, top, bottom, full-front — map the user's wording "
        "(VN or EN, e.g. 'ngực trái' → left-chest). Flat batches (no scene) auto-vary "
        "placement/scale per variant. Each ready variant in the result contains a `url` — "
        "render it inline: ![variant](url) — plus the `placement`/`design_scale` it was "
        "rendered with.\n\n"
        "4. **refine_mockups(job_id, design_id, product_id, variants, delta, placement)** — "
        "call this whenever the user asks to CHANGE an existing mockup (size, scene, product). "
        "Pass the `variants` list exactly as step 3 returned it (including each variant's "
        "`placement` and `design_scale`). `delta.type`: design (resize/move design, reuses "
        "scene) | scene (regenerate scene) | product (swap garment). On a scene delta, "
        "describe WHERE in `delta.setting` and WHO in `delta.model_persona` — e.g. "
        "'đổi sang mẫu nam' / 'change to a male model' → delta={'type': 'scene', "
        "'model_persona': 'a young man'}; unset fields keep the variant's previous "
        "setting/persona. A scene delta with neither field is rejected. "
        "`delta.scale` 0.3–1.6 resizes the printed design. Refines ONE image by "
        "default via `delta.target_ordinal` (1-based index of the variant to refine; default "
        "to the most recent) — omit target_ordinal ONLY when the user asks to update ALL "
        "images.\n\n"
        ...
        "- If the user wants a person/model WEARING the product ('a man wearing it', "
        "'người mẫu mặc nó'), you MUST pass scene_specs=[{'setting': '<location>', "
        "'model_persona': '<short persona, e.g. a young woman>'}] — the result is "
        "ai-rendered (near-exact print, disclose this). For a scene WITHOUT a person "
        "('on a beach'), pass only {'setting': ...}. Empty scene_specs renders a flat "
        "product-only image.\n"
        "- Any follow-up asking to change an existing mockup ('make it smaller', 'different "
        "scene', 'put it on a hoodie') → call refine_mockups with the previous variants — "
        "do NOT call generate_mockups again and do NOT answer with text only.\n"
